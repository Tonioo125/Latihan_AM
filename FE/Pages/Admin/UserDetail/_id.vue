    ...
    <div class="container mx-auto p-8">
        ...
        <div v-else class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="bg-white shadow-md rounded-3xl w-full lg:w-2/3 p-6">
                <h2 class="text-xl text-center font-semibold mb-6">Pending Member Profile</h2>
                <hr class="mb-4" />
                ...
                <button @click="$router.push('/admin/dashboard')"
                    class="bg-indigo-600 text-white px-6 py-2 rounded-full mt-8">
                    Back to Dashboard
                </button>
            </div>
            ...
            <div class="bg-white shadow-md rounded-3xl w-full lg:w-1/3 p-6">
                ...
                <hr class="mb-4" />
                <div class="" v-if="user.files && user.files.length > 0">
                    <ul class="flex justify-center flex-col space-y-2">
                        <li v-for="(file, index) in user.files" :key="index">
                            ...
                            <button @click="showFileDetails(file)"
                                class="flex items-center justify-center relative border-2 border-indigo-600 text-darkblue font-semibold px-32 w-full h-20 rounded-xl">
                                <div
                                    class="absolute left-3 w-16 h-16 mr-4 rounded-full bg-indigo-200 p-1 flex items-center justify-center">
...
import defaultPhoto from '../../../Components/Assets/default.svg'
...
export default {
    ...
    methods: {
        ...
        getUserPhotoUrl(user) {
            const photoFile = user.files?.find(f => f.file_category?.name === 'Photo');

            if (photoFile) {
                return `${this.$axios.defaults.baseURL}/storage/${photoFile.file_url}`;
            }
            ...
            <p>
                <a href="${downloadUrl}" 
                   ...
                   style="display: inline-block; background-color: #4F46E5; color: white;
                    padding: 0.5rem 1rem; border-radius: 9999px; text-decoration: none; margin-top: 1rem;">
                    Download File
                </a>
            </p>
        `,
